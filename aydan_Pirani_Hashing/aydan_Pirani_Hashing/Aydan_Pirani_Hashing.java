package aydan_Pirani_Hashing;

import java.util.LinkedList;

public class Aydan_Pirani_Hashing {
	public static void main(String[] args) {
		...
		map.put("Cook", 29);
		map.put("Smith", 65);
		System.out.println("Size after adding 5 entries:");
...
class MyHashMap<K, V> implements MyMap<K, V> {
	...
	public V put(K key, V value) {
		if (get(key) != null) { // The key is already in the map
			int bucketIndex = hash(key.hashCode());
			int repetitionsIndex = 0;
			boolean repNeeded = true;
			LinkedList<Entry<K, V>> bucket = table[bucketIndex]; 
				do {
					repetitionsIndex++;
					if (bucket.size() == 0) {
					...
					bucketIndex += (int) Math.pow(repetitionsIndex, 2);
				} while (repetitionsIndex < capacity/2 && repNeeded);	
		}
